    ...
    if key not in existing_keys:
        ...
        for p in data["projects"]:
            if p["key"] == key:
                ...
                print(f"   ℹ️  Project '{key}' already exists — updated links only")
                break
...
def update_github_profile_readme(state: AgentState) -> AgentState:
    ...
    try:
        ...
        print(f"   ⚠️  Could not fetch profile README: {e}")
        return state
    ...
    def replace_section(content, start_marker, end_marker, new_block):
        ...
        if start_idx == -1:
            print(f"   ⚠️  Section '{start_marker}' not found in README — skipping")
            return content
    ...
    if updated == current:
        print("   ℹ️  No changes to profile README — skipping commit")
        return state
    ...
    try:
        ...
        print(f"   ⚠️  Could not commit profile README: {e}")
...
def translate_content(state: AgentState) -> AgentState:
    ...
    if key in (en_data.get("projects", {}).get("items") or {}):
        print(f"   ℹ️  Key '{key}' already in en.json — skipping translation")
    ...
    async def translate_one(lang_code: str, lang_name: str) -> tuple[str, dict]:
        try:
            ...
            print(f"   ⚠️  {lang_code} translation failed: {e}")
            # Fallback to English
            return lang_code, {
                "title": state["project_title"],
                "description": state["project_description"],
...
def update_i18n_files(state: AgentState) -> AgentState:
    ...
    for lang_code, translated in translations.items():
        ...
        try:
            ...
            print(f"   ⚠️  Could not update {lang_code}.json: {e}")
